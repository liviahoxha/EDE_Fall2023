...
# Here I apply the glimpse() function to reveal dimensions, column names, and structure for the datasets
...
# Here I convert date columns to date objects for each of the datasets
...
# Now we select the specified columns for each of the datasets
...
# Here, for the PM2.5 datasets, we fill all cells in AQS_PARAMETER_DESC with "PM2.5"
...
# Here I check and make sure that the column names are identical across the datasets
...
#colnames(selected_data3) == colnames(selected_data4))) 
  ...
  # Now I combine the four datasets using rbind
...
# Here I define the list of common sites
common_sites <- c(
...
# Now I filter only the common sites
filtered_data <- combined_data %>%
  filter(Site.Name %in% common_sites)



# Here I group by date, site name, AQS parameter, and county, and calculate daily means
...
#  Now I add columns for Month and Year
...
# Here I check the dimensions of the final dataset, which should be 14,752 x 9
dim(grouped_data) 
...
# Here I spread the data to have AQI values for ozone and PM2.5 in separate columns
...
# Now I check the structure of the spread_data
glimpse(spread_data)

#10

# Also, here I call up the dimensions of the dataset
dim(spread_data)


#11

# Now, I save my processed dataset with the specified file name
...
# Here I group the data by Site Name, Year, and Month
summary_data <- spread_data %>%
  group_by(Site.Name, Year, Month) %>%
  
  # Now I calculate the mean AQI values for ozone and PM2.5
  summarize(
...
> Answer: We used the `drop_na` function over `na.omit` because `drop_na` allows us to selectively remove rows with missing values based on specific columns, offering more flexibility and customization. In our case, we wanted to retain rows with missing PM2.5 values while removing rows with missing ozone values, aligning with our data processing requirements.
